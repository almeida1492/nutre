...
public class UserInfoContainer {
    ...
    private final static String ENERGY_KEY= "ENERGY_KEY";
    ...
    public static double getEnergy(Context c){
        SharedPreferences sp = c.getSharedPreferences(USER_INFO_CONTAINER, Context.MODE_PRIVATE);
        if(sp.getInt(ENERGY_KEY,0)==0){
            setEnergy(c,0);
        }
        return sp.getInt(ENERGY_KEY,0);
    }

    public static void setEnergy(Context c, double energy){
        SharedPreferences sp = c.getSharedPreferences(USER_INFO_CONTAINER, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sp.edit();
        editor.putInt(ENERGY_KEY, (int) energy);
        editor.apply();
    }
